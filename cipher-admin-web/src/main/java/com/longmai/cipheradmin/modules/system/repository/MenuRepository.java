...
public interface MenuRepository extends JpaRepository<Menu, Long>, JpaSpecificationExecutor<Menu> {
     ...
     */
    @Query(value = "SELECT m.* FROM sys_permission m, sys_role_permission r WHERE " +
            "m.id = r.permission_id AND r.role_id IN ?1 AND type != ?2 order by m.menu_sort asc",nativeQuery = true)
     ...
     */
    @Modifying
    @Query(value = " update sys_permission set sub_count = ?1 where id = ?2 ",nativeQuery = true)
